...
#The TSObjectiveFunctionComponent is a time series objective function calculation component 

TEMPLATE = lib
VERSION = 1.0.0
TARGET = TSObjectiveFunctionComponent
QT -= gui
QT += testlib

DEFINES += TSOBJECTIVEFUNCTIONCOMPONENT_LIBRARY
DEFINES += USE_OPENMP
DEFINES += USE_MPI
DEFINES += USE_NETCDF
DEFINES += USE_CHPC
...
contains(DEFINES,TSOBJECTIVEFUNCTIONCOMPONENT_LIBRARY){
  TEMPLATE = lib
  message("Compiling TSObjectiveFunctionComponent as library")
} else {
  TEMPLATE = app
  CONFIG-=app_bundle
  message("Compiling TSObjectiveFunctionComponent as application")
}
